...
class Producto(Articulo):
    ...
    tipo_producto = models.CharField(max_length=30, choices=TIPOS)
...
class Venta(models.Model):
    ...
    medio_pago = models.CharField(max_length=30, choices=MEDIOS)
    administrador = models.ForeignKey(Administrador, on_delete=models.CASCADE, null=True)
    num_tarjeta = models.BigIntegerField(
        null=True, blank=True, validators=[MaxValueValidator(9999999999999999), MinValueValidator(1111111111111111)]
    )
...
class RutinaCliente(models.Model):
    ...
    def save(self, *args, **kwargs):
        if self.actual:
            RutinaCliente.objects.filter(actual=True).update(actual=False)
